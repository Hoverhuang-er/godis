...
```
main.go
  ├── config (redis.conf parser, ServerProperties)
  ├── cluster or database.Server
  │     ├── database.DB (single DB instance)
  │     │     ├── datastruct/dict (ConcurrentDict, sharded map)
  │     │     ├── datastruct/list, set, sortedset, bitmap, stream, ...
  │     │     ├── lib/timewheel (TTL expiry)
  │     │     └── datastruct/lock (per-key RWMutex table)
  │     ├── pubsub.Hub
  │     └── aof.Persister (AOF + RDB preamble)
  ├── redis/server/std or redis/server/gnet (TCP layer)
  │     ├── redis/parser (RESP parser: ParseStream / ParseV2)
  │     ├── redis/protocol (reply builders)
  │     └── redis/connection (Connection impl, sync.Pool)
  └── interface/ (redis.Reply, redis.Connection, database.DB, tcp.Handler)
```
...
| Directory | Purpose |
|-----------|---------|
| `interface/` | Contract types: `redis.Reply`, `redis.Connection`, `database.DB`, `database.DBEngine`, `tcp.Handler` |
| `database/` | Core server logic: `DB` (single DB), `Server` (multi-DB), per-type command files (`string.go`, `hash.go`, `list.go`, `set.go`, `sortedset.go`, `geo.go`, `stream.go`, `timeseries.go`, `search.go`) |
| `datastruct/` | In-memory data structure implementations — dict, list, set, sortedset, bitmap, stream, lock, timeseries, search, array |
| `redis/` | Protocol layer — `parser/` (RESP parsing), `protocol/` (reply encoding), `connection/` (client conn), `server/` (std + gnet handlers), `client/` (pipeline client) |
...
| `lib/` | Utility packages (arena, pool, logger, wildcard, timewheel, idgenerator, consistenthash, geohash, sync, utils, greenteagc) |
...
| Command | Purpose |
|---------|---------|
| `go build -o godis main.go` | Build binary |
| `go run main.go` | Run with default config (redis.conf) |
| `CONFIG=my.conf go run main.go` | Run with custom config |
| `go test ./...` | Run all tests |
| `go test -v -coverprofile=profile.cov ./...` | Tests with coverage |
| `go test -run TestSet ./database` | Run specific test |
| `./build.sh` | Cross-compile release (9 OS/arch targets, v1.3.0) |
| `./build-all.sh` | Quick cross-compile (5 targets) |
...
- **Alias types**: `type CmdLine = [][]byte` (declared in both `database/` and `aof/`)
...
Structured JSON logging via `lib/logger` (wraps `zap.Logger`, bridges `slog`). Global `DefaultLogger` with package-level convenience functions. Writes to stdout + `godis.log`.
...
| File | Purpose |
|------|---------|
| `main.go` | Entry point: config → cluster or standalone → TCP listener |
| `database/database.go` | Core `DB` struct: data store, TTL, version map, command dispatch |
| `database/server.go` | Multi-DB `Server` struct: replication, AOF, pub/sub, system commands |
| `database/router.go` | Command registration table (`cmdTable`), `registerCommand()`, flag definitions |
| `interface/redis/reply.go` | `Reply` interface — single `ToBytes() []byte` method |
| `interface/redis/conn.go` | `Connection` interface — full client contract |
| `interface/database/db.go` | `DB` / `DBEngine` interfaces, `DataEntity` struct |
| `redis/parser/parser.go` | `ParseStream` — channel-based RESP parser |
| `redis/protocol/reply.go` | RESP2 reply builders (`BulkReply`, `MultiBulkReply`, etc.) |
| `redis/protocol/consts.go` | Singleton replies (`PongReply`, `OkReply`, `NullBulkReply`) |
| `redis/connection/conn.go` | Connection implementation (`sync.Pool` recycled) |
| `cluster/cluster.go` | Cluster entry point (alias to `core.Cluster`) |
| `config/config.go` | `ServerProperties` with reflect-based config parsing |
| `aof/aof.go` | AOF persistence — background writer, rewrite, RDB preamble |
...
- **Test helpers**: `MakeTestDB()` / `NewStandaloneServer()` create in-memory test fixtures; `Exec()` returns `redis.Reply`; assertions compare `ToBytes()` output or use `redis/protocol/asserts` helpers
- **Test patterns**:
...
- **Release CI**: GitHub Actions on `v*` tags — matrix build 9 targets, creates release with artifacts and checksums
